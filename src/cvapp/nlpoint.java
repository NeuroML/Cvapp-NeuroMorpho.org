...
class nlpoint extends Object {
   ...
   public void print() {
      System.out.println (" nlpoint " + this +" " +  myIndex + " " + 
			  x + " " + y + " " + z);
   ...
   public void addNeighbor (nlpoint p) {
	 ...
	 if (nnbr >= pnbr.length) {
	    ...
	    for (int i = 0; i < pnbr.length; i++) newpnbr[i] = pnbr[i];
	    pnbr = newpnbr;
	 }
	 pnbr[nnbr] = p;
   ...
   public boolean removeNeighbor (nlpoint p) {
	 ...
	 for (i = 0; !got && i < nnbr; i++) {
	    if (pnbr[i] == p) {
	       ...
	       for (j = 0; j < i; j++) newpnbr[j] = pnbr[j];
	       for (j = i; j < nnbr-1; j++) newpnbr[j] = pnbr[j+1];
	       pnbr = newpnbr;
	       nnbr--;
	    }
	 }
