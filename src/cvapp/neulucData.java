...
class neulucData extends Object {
   ...
   public int getCode (String s) {
	 ...
	 if (s.equals (sectionTypes [i])) icode = i;
      }
   ...
   public void setHeaderText (String s) {
    ...
    public void updateHeaderText (String s) {
       ...
       for (int i = 0; i < nfo; i++) {
        ...
        if (stt.hasMoreTokens()) fao[i] = stt.nextToken();
       }
   ...
   public double[] getScale () {
	 ...
	 if ((n = s.indexOf("SCALE")) > 0) {
	    ...
	    StringTokenizer st1 = new 
                        StringTokenizer (s.substring (n+5,s.length()));
   ...
   private void checkHeader () {
	 ...
	 if (s.startsWith("#")) {
	    ...
	    for (int j = 0; j < nhf; j++) {
	       if (s.indexOf(headerField[j]) > 0) bgot[j] = true;
	       if (s.indexOf("SCALE") > 0) gotShrink = true;
	    }
	 }
	 if (s.startsWith ("#")) {
	    sb.append (s);
	 ...
	 for (int i = 0; i < nhf; i++) {
      ...
      if (!gotShrink) sb.append ("# SCALE 1.0 1.0 1.0 \n");
      headerText = sb.toString();
   }
  ...
  public String write () {
   ...
   public void HOCtrace (int ip) {
	 ...
	 rootpoint = ip;
	 for (int i = 0; i < np; i++) done[i] = false;
   ...
   private int maxRadiusPoint() {
	 ...
	 if (ptmp.nlcode > maxType) maxType = ptmp.nlcode;
      }
      return rp;
   }
...
//	 System.out.println ("assuming join " + rbest + " " + ibest);
   ...
   public void recAddLine (nlpoint p, int[] nl, boolean[] done) {
	 ...
	 if (!done[q.myIndex]) {
	    ...
	    if (p.nlcode ==  q.nlcode) lineList[k][9] = p.nlcode + 0.1;
	    nl[0]++;
   ...
   public double[][] getPointList () {
	 ...
	 for (int i = 0; i < np; i++) {
	    ...
	    if (p.identPoint != null) pointList[i][4] = 2;
	 }
	 trustPointList = true;
      }
      return pointList;
...
//      pointList[p.myIndex][3] = 1.0;
	 ...
	 if (SWCwrite) {
	    ...
	    sb.append (" " + p.writeIndex + " " + p.nlcode  + " "); 
	     sb.append (form.format(p.x)); sb.append(" ");
	     sb.append (form.format(p.y)); sb.append(" ");
	     sb.append (form.format(p.z)); sb.append(" ");
	     sb.append (form.format(p.r)); sb.append(" ");
	 ...
	 if (GENESISwrite) {
	    if (p.identPoint == null) {
		  ...
		  sb.append (" none ");
	       ...
	       sb.append (form.format(p.y)); sb.append(" ");
	       sb.append (form.format(p.z)); sb.append(" ");
	       sb.append (form.format(2. * p.r)); sb.append("\n");
	    }
	 }
